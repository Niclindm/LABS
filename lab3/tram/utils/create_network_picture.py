...
MY_TRAMNETWORK_JSON = 'static/tramnetwork.json'  # JSON file from lab1
TRAM_URL_FILE = 'static/tram-url.json'  # given in lab3/files, replace with your own in bonus 2
...
def stop_url(stop):
    ...
    return stop_urls[stop]['timetable']
...
def network_graphviz(network, outfile=MY_GBG_SVG, positions=scaled_position):
    ...
    for stop in network.all_stops():
        ...
        dot.node(stop, label=stop, shape='rectangle', pos=pos_x + ',' + pos_y +'!',
            fontsize='8pt', font="Arial" , width='0.4', height='0.05',
            URL=stop_url(stop),
        fillcolor=col, style='filled', background='purple')
